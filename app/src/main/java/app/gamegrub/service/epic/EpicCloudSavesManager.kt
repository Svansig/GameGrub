...
object EpicCloudSavesManager {
    ...
    private fun packageSaveFiles(
        ...
        try {
            ...
            val chunks = mutableListOf<app.gamegrub.service.epic.manifest.ChunkInfo>()
            ...
            files.forEach { file ->
                try {
                    ...
                    val fileManifest = app.gamegrub.service.epic.manifest.FileManifest()
    ...
    private fun finalizeChunk(
    ...
    ): app.gamegrub.service.epic.manifest.ChunkInfo {
        ...
        val chunkInfo = app.gamegrub.service.epic.manifest.ChunkInfo()
        chunkInfo.guid = guid
    ...
    private fun createManifest(
        game: EpicGame,
        accountId: String,
        chunks: List<app.gamegrub.service.epic.manifest.ChunkInfo>,
        fileManifests: List<app.gamegrub.service.epic.manifest.FileManifest>,
    ): EpicManifest {
        val manifest = app.gamegrub.service.epic.manifest.BinaryManifest()

        // Meta
        manifest.meta = app.gamegrub.service.epic.manifest.ManifestMeta()
        ...
        // Custom fields
        manifest.customFields = app.gamegrub.service.epic.manifest.CustomFields()
        manifest.customFields!!["CloudSaveFolder"] = game.saveFolder

        // Chunks
        manifest.chunkDataList = app.gamegrub.service.epic.manifest.ChunkDataList()
        manifest.chunkDataList!!.elements.addAll(chunks)

        // Files
        manifest.fileManifestList = app.gamegrub.service.epic.manifest.FileManifestList()
