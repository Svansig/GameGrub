...
class EpicDownloadManager @Inject constructor(
    ...
    private suspend fun downloadChunkWithRetry(
        chunk: app.gamegrub.service.epic.manifest.ChunkInfo,
        chunkCacheDir: File,
        chunkDir: String,
        ...
        repeat(MAX_CHUNK_RETRIES) { attempt ->
            ...
            if (attempt < MAX_CHUNK_RETRIES - 1) {
                ...
                Timber.tag("Epic")
                    .w("Chunk ${chunk.guidStr} download failed (attempt ${attempt + 1}/$MAX_CHUNK_RETRIES): ${lastException?.message}. Retrying in ${delay}ms...")
    ...
    private suspend fun downloadChunk(
        ...
        try {
            File(chunkCacheDir, "${chunk.guidStr}.chunk")
    ...
    private fun decompressStreamingChunkToFile(
        ...
        chunkFile.inputStream().buffered().use { input ->
            ...
            val uncompressedSize = if (headerVersion >= 3 && buffer.remaining() >= 4) {
                ...
                Timber.tag("Epic")
                    .d("Header version $headerVersion doesn't include uncompressedSize field, using expectedSize=$expectedSize")
                expectedSize.toInt()
            }

            Timber.tag("Epic").d(
                "Chunk header: magic=0x${magic.toString(16)}, headerVersion=$headerVersion, headerSize=$headerSize, compressedSize=$compressedSize, uncompressedSize=$uncompressedSize, storedAs=0x${
                    storedAs.toString(16)
                }, isCompressed=$isCompressed, expectedSize=$expectedSize",
            )
        ...
        if (totalBytesWritten != expectedSize) {
            Timber.tag("Epic")
                .d("Size mismatch: expected=$expectedSize, actual=$totalBytesWritten, diff=${expectedSize - totalBytesWritten}")
            outputFile.delete()
    ...
    private suspend fun assembleFile(
        fileManifest: app.gamegrub.service.epic.manifest.FileManifest,
        chunkCacheDir: File,
        installDir: File,
    ): Result<File> = withContext(Dispatchers.IO) {
        try {
