...
class EpicManager @Inject constructor(
    ...
    data class ManifestResult(
        ...
        override fun hashCode(): Int {
            ...
            result = 31 * result + cdnUrls.hashCode()
            return result
        }
    }
    ...
    suspend fun fetchManifestSizes(context: Context, appId: Int): ManifestSizes = withContext(Dispatchers.IO) {
        try {
            ...
            val manifest = app.gamegrub.service.epic.manifest.EpicManifest.readAll(manifestData.manifestBytes)
            ...
            val (downloadSize, installSize) = app.gamegrub.service.epic.manifest.ManifestUtils.getSizesForSelectedInstallTags(
                manifest,
                emptyList(),
            )
            Timber.tag("Epic").d(
