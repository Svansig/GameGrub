...
object GOGApiClient {
    ...
    private fun transformGameDetails(rawResponse: JSONObject, gameId: String): ParsedGogGame {
        ...
        val publisher = when (val publisherObj = rawResponse.opt("publisher")) {
        ...
        if (genresArray != null) {
            for (i in 0 until genresArray.length()) {
                val genreName = when (val genreObj = genresArray.opt(i)) {
        ...
        val description = when (val descriptionObj = rawResponse.opt("description")) {
