...
class GOGDownloadManager @Inject constructor(
    ...
    suspend fun downloadGame(
        ...
        try {
            ...
            Timber.tag("GOG")
                .i("Selected build: ${selectedBuild.buildId} (Gen ${selectedBuild.generation}, Platform: ${selectedBuild.platform})")
            ...
            depots.forEachIndexed { index, depot ->
                Timber.tag("GOG")
                    .d("  Depot $index: productId=${depot.productId}, manifest=${depot.manifest}, size=${depot.size}, compressedSize=${depot.compressedSize}")
            }
    ...
    private suspend fun downloadGameGen1(
        ...
        downloadInfo: DownloadInfo,
        gameManifest: GOGManifestMeta,
        selectedBuild: app.gamegrub.service.gog.api.GOGBuild,
        language: String,
        withDlcs: Boolean,
        supportDir: File?,
    ): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            val timestamp =
                gameManifest.productTimestamp ?: return@withContext Result.failure(Exception("Gen 1 manifest missing productTimestamp"))
            ...
            gameFiles.size + if (supportDir != null) supportFiles.size else 0
            var doneFiles = 0
        ...
        try {
            ...
            for ((index, depot) in filteredDepots.withIndex()) {
                ...
                if (depotManifestResult.isFailure) {
                    Timber.tag("GOG")
                        .w("Failed to fetch depot manifest for ${depot.readableName}: ${depotManifestResult.exceptionOrNull()?.message}")
                    continue
                }
