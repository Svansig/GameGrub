    ...
    fun filterDepotsByLanguage(manifest: GOGManifestMeta, containerLanguage: String): Pair<List<Depot>, String> {
        ...
        for (fallbackLang in englishCodes) {
            ...
            if (matched.isNotEmpty()) {
                val result = (matched + starDepots).distinct()
                Timber.tag(TAG)
                    .d("No depots for requested language codes, fell back to $fallbackLang: ${result.size}/${manifest.depots.size} depots")
        ...
        Timber.tag(TAG)
            .d("No language match for $containerLanguage, using all ${manifest.depots.size} depots with effective: $effectiveLang")
