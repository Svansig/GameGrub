...
internal fun PhysicalControllerConfigSection(
        ...
        if (ctrl == null) {
            ...
            if (defaultProfile != null) {
                ...
                if (defaultControllers.isNotEmpty()) {
                    ...
                    homeButtonBinding.binding = Binding.OPEN_NAVIGATION_MENU
                    // Remove any existing home button binding first
                    val existingHomeBinding = ctrl.controllerBindings.find {
                        it.keyCodeForAxis == KeyEvent.KEYCODE_BUTTON_MODE
                    }
    ...
    val workingBindings = remember { mutableStateMapOf<Int, Binding?>() }

    // Initialize working copy with current bindings
    LaunchedEffect(controller) {
        controller?.controllerBindings?.forEach {
            workingBindings[it.keyCodeForAxis] = it.binding
        }
    }
                            ...
                            onClick = {
                                ...
                                if (defaultProfile != null) {
                                    ...
                                    if (defaultControllers.isNotEmpty()) {
                                        val defaultController = defaultControllers[0]
                                        for (binding in defaultController.controllerBindings) {
                                            workingBindings[binding.keyCodeForAxis] = binding.binding
                                        }
                                    }
                                }
