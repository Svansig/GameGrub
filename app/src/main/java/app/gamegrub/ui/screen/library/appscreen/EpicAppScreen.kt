...
class EpicAppScreen : BaseAppScreen() {
    ...
    companion object {
        ...
        private val gameManagerDialogStates = mutableStateMapOf<Int, app.gamegrub.ui.component.dialog.state.GameManagerDialogState>()

        fun showGameManagerDialog(gameId: Int, state: app.gamegrub.ui.component.dialog.state.GameManagerDialogState) {
        ...
        fun getGameManagerDialogState(gameId: Int): app.gamegrub.ui.component.dialog.state.GameManagerDialogState? {
    ...
    override fun getGameDisplayInfo(
        ...
        val releaseDateTimestamp = if (game?.releaseDate?.isNotEmpty() == true) {
            try {
                ...
                Timber.tag(TAG)
                    .d("Parsed release date '${game.releaseDate}' -> $timestampSeconds seconds (${java.util.Date(timestampMillis)})")
                timestampSeconds
            } catch (e: Exception) {
    ...
    override fun onDownloadInstallClick(context: Context, libraryItem: LibraryItem, onClickPlay: (Boolean) -> Unit) {
        ...
        Timber.tag(TAG)
            .d("onDownloadInstallClick: appId=${libraryItem.appId}, gameId=$gameId, isDownloading=$isDownloading, installed=$installed")
        ...
        if (isDownloading) {
            ...
            showGameManagerDialog(
                gameId,
                app.gamegrub.ui.component.dialog.state.GameManagerDialogState(visible = true),
            )
        }
    }
    ...
    override fun observeGameState(
        ...
        val installListener: (app.gamegrub.events.AndroidEvent.LibraryInstallStatusChanged) -> Unit = { event ->
            Timber.tag(TAG)
                .d("[OBSERVE] LibraryInstallStatusChanged event received: event.appId=${event.appId}, libraryItem.appId=${libraryItem.appId}, match=${event.appId == libraryItem.gameId}")
            if (event.appId == libraryItem.gameId) {
    ...
    override fun AdditionalDialogs(
        ...
        var gameManagerDialogState by remember(gameId) {
            mutableStateOf(getGameManagerDialogState(gameId) ?: app.gamegrub.ui.component.dialog.state.GameManagerDialogState(false))
        }
        LaunchedEffect(gameId) {
            snapshotFlow { getGameManagerDialogState(gameId) }
                .collect { state ->
                    gameManagerDialogState = state ?: app.gamegrub.ui.component.dialog.state.GameManagerDialogState(false)
                }
        }
        ...
        if (gameManagerDialogState.visible) {
            app.gamegrub.ui.component.dialog.EpicGameManagerDialog(
                visible = true,
