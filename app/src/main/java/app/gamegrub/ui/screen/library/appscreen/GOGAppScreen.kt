...
class GOGAppScreen : BaseAppScreen() {
    ...
    override fun getGameDisplayInfo(
        ...
        // Extract numeric gameId for GOG domain calls
        ...
        LaunchedEffect(gameId, refreshTrigger) {
            gogGame = getGOGInstallDomain(context).getGOGGameOf(gameId)
        }
        ...
        Timber.tag(TAG)
            .d("Returning GameDisplayInfo: name=${displayInfo.name}, iconUrl=${displayInfo.iconUrl}, heroImageUrl=${displayInfo.heroImageUrl}, developer=${displayInfo.developer}, installLocation=${displayInfo.installLocation}")
        return displayInfo
    }
    ...
    override fun isDownloading(context: Context, libraryItem: LibraryItem): Boolean {
        ...
        Timber.tag(TAG)
            .d("isDownloading: appId=${libraryItem.appId}, hasDownloadInfo=${downloadInfo != null}, active=$isActive, progress=$progress, result=$downloading")
        return downloading
    }
    ...
    override fun onDownloadInstallClick(context: Context, libraryItem: LibraryItem, onClickPlay: (Boolean) -> Unit) {
        ...
        // GOGService expects numeric gameId
        val gameId = libraryItem.gameId.toString()
        val downloadInfo = getGOGInstallDomain(context).getDownloadInfo(gameId)
        ...
        if (isDownloading) {
            ...
            downloadInfo?.cancel()
            getGOGInstallDomain(context).cleanupDownload(gameId)
        } else if (installed) {
    ...
    private fun showGOGInstallConfirmationDialog(context: Context, libraryItem: LibraryItem) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val game = getGOGInstallDomain(context).getGOGGameOf(gameId)
                ...
                showInstallDialog(libraryItem.appId, state)
            } catch (e: Exception) {
    ...
    private fun performDownload(context: Context, libraryItem: LibraryItem, onClickPlay: (Boolean) -> Unit) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // Start download - GOGInstallDomain will handle monitoring, database updates, verification, and events
                val result = getGOGInstallDomain(context).downloadGame(gameId, installPath, containerData.language)
    ...
    override fun onDeleteDownloadClick(context: Context, libraryItem: LibraryItem) {
        ...
        Timber.tag(TAG)
            .d("onDeleteDownloadClick: appId=${libraryItem.appId}, isDownloading=$isDownloadingFlag, isInstalled=$isInstalledFlag, hasPartial=$hasPartial")
    ...
    private fun performUninstall(context: Context, libraryItem: LibraryItem) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                // Delegate to GOGInstallDomain which calls GOGService
                val result = getGOGInstallDomain(context).deleteGame(context, libraryItem)
    ...
    override fun saveContainerConfig(context: Context, libraryItem: LibraryItem, config: ContainerData) {
        ...
        if (previousLanguage != config.language) {
            CoroutineScope(Dispatchers.IO).launch {
                val gameId = libraryItem.gameId.toString()
                if (!getGOGInstallDomain(context).isGameInstalled(gameId)) return@launch
                if (getGOGInstallDomain(context).getDownloadInfo(gameId)?.isActive() == true) return@launch

                val installPath = getGOGInstallDomain(context).getInstallPath(gameId)
                    ?: GOGConstants.getGameInstallPath(libraryItem.name)

                getGOGInstallDomain(context).downloadGame(gameId, installPath, config.language)
            }
        }
    }
    ...
    override fun observeGameState(
        ...
        val downloadStatusListener: (app.gamegrub.events.AndroidEvent.DownloadStatusChanged) -> Unit = { event ->
            Timber.tag(TAG)
                .d("[OBSERVE] DownloadStatusChanged event received: event.appId=${event.appId}, libraryItem.gameId=${libraryItem.gameId}, match=${event.appId == libraryItem.gameId}")
            if (event.appId == libraryItem.gameId) {
        ...
        val installListener: (app.gamegrub.events.AndroidEvent.LibraryInstallStatusChanged) -> Unit = { event ->
            Timber.tag(TAG)
                .d("[OBSERVE] LibraryInstallStatusChanged event received: event.appId=${event.appId}, libraryItem.gameId=${libraryItem.gameId}, match=${event.appId == libraryItem.gameId}")
            if (event.appId == libraryItem.gameId) {
    ...
    override fun AdditionalDialogs(
        ...
        var installDialogState by remember(appId) {
            mutableStateOf(getInstallDialogState(appId) ?: app.gamegrub.ui.component.dialog.state.MessageDialogState(false))
        }
        LaunchedEffect(appId) {
            snapshotFlow { getInstallDialogState(appId) }
                .collect { state ->
                    installDialogState = state ?: app.gamegrub.ui.component.dialog.state.MessageDialogState(false)
                }
        }
        ...
        if (installDialogState.visible) {
                ...
                app.gamegrub.ui.enums.DialogType.CANCEL_APP_DOWNLOAD -> {
                    {
                        ...
                        CoroutineScope(Dispatchers.IO).launch {
                            ...
                            val isInstalledAfterCancel = getGOGInstallDomain(context).isGameInstalled(gameId)
            ...
            app.gamegrub.ui.component.dialog.MessageDialog(
