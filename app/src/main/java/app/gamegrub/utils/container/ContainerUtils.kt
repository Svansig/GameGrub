package app.gamegrub.utils.container
...
object ContainerUtils {
    ...
    fun toContainerData(container: Container): ContainerData {
        ...
        val apiOrdinal = container.inputType
        val enableX = apiOrdinal == WinHandler.PreferredInputApi.XINPUT.ordinal || apiOrdinal == WinHandler.PreferredInputApi.BOTH.ordinal
        val enableD = apiOrdinal == WinHandler.PreferredInputApi.DINPUT.ordinal || apiOrdinal == WinHandler.PreferredInputApi.BOTH.ordinal
        val mapperType = container.dinputMapperType
        ...
        val gestureConfig = container.gestureConfig
        val externalDisplayMode = container.externalDisplayMode
    ...
    fun applyBestConfigMapToContainerData(containerData: ContainerData, bestConfigMap: Map<String, Any?>): ContainerData {
        ...
        bestConfigMap.forEach { (key, value) ->
            updatedData = when (key) {
                ...
                "videoMemorySize" -> value?.let { updatedData.copy(videoMemorySize = it as? String ?: updatedData.videoMemorySize) }
                    ?: updatedData
    ...
    fun applyToContainer(context: Context, container: Container, containerData: ContainerData, saveToDisk: Boolean) {
        ...
        container.fexCorePreset = containerData.fexcorePreset
        container.isDisableMouseInput = containerData.disableMouseInput
        container.isTouchscreenMode = containerData.touchscreenMode
        container.isShooterMode = containerData.shooterMode
        container.gestureConfig = containerData.gestureConfig
        container.externalDisplayMode = containerData.externalDisplayMode
        container.isExternalDisplaySwap = containerData.externalDisplaySwap
        container.isForceDlc = containerData.forceDlc
        container.isSteamOfflineMode = containerData.steamOfflineMode
        container.isUseLegacyDRM = containerData.useLegacyDRM
        container.isUnpackFiles = containerData.unpackFiles
        container.suspendPolicy = containerData.suspendPolicy
        container.isPortraitMode = containerData.portraitMode
        ...
        val api = when {
            containerData.enableXInput && containerData.enableDInput -> WinHandler.PreferredInputApi.BOTH
            containerData.enableXInput -> WinHandler.PreferredInputApi.XINPUT
            containerData.enableDInput -> WinHandler.PreferredInputApi.DINPUT
            else -> WinHandler.PreferredInputApi.AUTO
        }
