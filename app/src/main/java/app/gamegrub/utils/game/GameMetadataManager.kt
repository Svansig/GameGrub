...
object GameMetadataManager {
    ...
    fun write(folder: File, metadata: GameMetadata) {
        ...
        try {
            ...
            Timber.tag("GameMetadataManager")
                .d("Wrote metadata to ${gamenativeFile.absolutePath}: appId=${merged.appId}, fetched=${merged.steamgriddbFetched}, releaseDate=${merged.releaseDate}")
        } catch (e: Exception) {
    ...
    fun update(folder: File, appId: Int? = null, steamgriddbFetched: Boolean? = null, releaseDate: Long? = null) {
        ...
        if (finalAppId == null) {
            Timber.tag("GameMetadataManager")
                .w("Cannot update metadata: no appId provided and no existing metadata found for ${folder.absolutePath}")
            return
        }
