...
/**
 * Atom - X11 atom (string identifier).
 * 
 * An Atom is an interned string identifier in X11.
 * Used for property names, window manager protocols, etc.
 * 
 * Instead of passing string names in requests, X11 uses integer
 * atom IDs for efficiency. Atoms are interned (registered once).
 * 
 * Predefined atoms (built-in):
 * - WM_* (window manager properties)
 * - XA_* (standard types)
 * - CUT_BUFFER* (selection data)
 * 
 * @see <a href="https://www.x.org/wiki/X11/">X11 Atoms</a>
 */
